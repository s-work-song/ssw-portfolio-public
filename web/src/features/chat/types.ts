 ...
 * `not_called`는 구버전 서버의 도구 미실행 보고와 호환하기 위한 값이다.
 * 현재 서버는 필수 도구가 끝내 없으면 응답을 오류로 닫는다.
 * 옛 서버는 이 필드를 보내지 않으므로 파서가 없으면 무시한다.
 */
export type ChatUiToolOutcome = "called" | "incomplete" | "not_called" | "not_required";
...
export interface ChatResponse {
  ...
  /** 설명 없이 화면 조작만 요청한 턴이다. 실제 결과 문구는 브라우저 관측값으로 만든다. */
  uiOnly?: boolean;
}
...
export interface ChatMessage {
  ...
  role: "user" | "assistant";
  content: string;
  kind: "greeting" | "message" | "retrieval_fallback" | "failure_explanation";
...
/** 변경 기록에 보존할 수 있는 설정 도구다. */
export type ChatSettingChangeToolName =
  | "set_portfolio_theme"
  | "set_portfolio_accent"
  | "set_portfolio_chat_layout"
  | "set_portfolio_chat_font"
...
/** 브라우저가 실제 반영을 확인한 설정 도구 호출 하나다. */
export interface ChatSettingChange {
  toolCallId: string;
  /** 같은 사용자 요청에서 확인된 설정 변경을 묶는다. */
  groupId: string;
  toolName: ChatSettingChangeToolName;
  before: string;
  after: string;
}
...
export interface ChatRequest {
  ...
  /** 이 브라우저에서 확인한 최근 설정 도구 변경만 최대 20개 보낸다. */
  uiSettingChanges: ChatSettingChange[];
