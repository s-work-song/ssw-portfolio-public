...
WebMCP를 지원하는 브라우저에서는 현재 화면 상태 확인, 설정 페이지 열기, 포인트 색상 순회, 포트폴리오 페이지·연구 연도 이동, 특정 연도 상세 펼치기·접기와 스트리밍 연출 변경 같은 화면 기능도 도구로 사용할 수 있습니다. 기록 검색과 상세 열기는 기록 페이지에서 직접 이용할 수 있습니다.
...
/** 사고모드 단추를 노출하지 않는다. 이전 ON 저장값도 Provider에서 OFF로 정리한다. */
...
export const UI_TOOL_INCOMPLETE_RESULT: ToolResult = {
  callId: "outcome",
  tool: "none",
  label: "화면 이동·설정 변경",
  status: "failed",
  detail: "요청한 조작이 모두 실행되지 않았어요",
};
