...
import {
  ...
  type PortfolioLogToolName,
} from '@/features/portfolio-tools/logs';
import type { ToolDefinition } from '@/features/portfolio-tools/contract';
...
/** 로그 도구의 안내 문구만 등록 청크에 둔다. 입력 계약은 logs 도메인에 있다. */
const LOG_TOOL_PRESENTATION: Readonly<Record<PortfolioLogToolName, {
  title: string;
  description: string;
  readOnly?: boolean;
}>> = {
  get_portfolio_log_outline: {
    ...
    description: '특정 공개 기록의 제목, 요약, 태그와 소제목별 section id를 반환합니다. 내용을 열기 전에 글의 구성을 확인할 때 사용하세요.',
    readOnly: true,
  },
  open_portfolio_log: {
    title: '포트폴리오 기록 열기',
    description: '공개 기록 상세 페이지 또는 검색 결과가 가리킨 소제목으로 이동하고 해당 위치를 강조합니다. 현재 기록 페이지나 목차 조회에서 확인한 slug와 section id를 사용하세요.',
  },
};

/**
 * 포트폴리오 도구 12종을 브라우저 모델 컨텍스트에 등록한다.
 *
...
export function PortfolioWebMcpTools() {
  ...
  useEffect(() => {
    ...
    /** 로그 정의와 브라우저 executor를 조립한다. 계약 자체는 logs 도메인이 소유한다. */
    const logTool = <TInput, TName extends PortfolioLogToolName>(
      definition: ToolDefinition<TInput, TName>,
      execute: (input: TInput) => Promise<unknown>,
    ): WebMcpTool => {
      const presentation = LOG_TOOL_PRESENTATION[definition.name];
      return {
        name: definition.name,
        title: presentation.title,
        description: presentation.description,
        inputSchema: definition.inputSchema,
        ...(presentation.readOnly ? { annotations: READ_ONLY_ANNOTATIONS } : {}),
        async execute(input) {
          try {
            return toolResult(await execute(definition.parse(input)));
          } catch (error) {
            return toolError(error);
          }
        },
    ...
    const tools: WebMcpTool[] = [
      uiTool('get_portfolio_ui_settings'),
      uiTool('set_portfolio_theme'),
      uiTool('set_portfolio_accent'),
      uiTool('set_portfolio_chat_layout'),
      uiTool('set_portfolio_chat_font'),
      uiTool('set_portfolio_chat_font_size'),
      logTool(GET_PORTFOLIO_LOG_OUTLINE_TOOL, async ({ slug }) => {
        return getPortfolioLogOutline(slug, controller.signal);
      }),
      logTool(OPEN_PORTFOLIO_LOG_TOOL, async ({ slug, sectionId }) => {
      ...
      uiTool('get_portfolio_view_state'),
      uiTool('control_portfolio_view'),
      uiTool('open_portfolio_settings'),
      uiTool('set_portfolio_stream_animation'),
    ];
