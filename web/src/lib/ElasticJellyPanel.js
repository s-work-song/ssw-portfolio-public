...
class ElasticJellyPanel {
  ...
  constructor(fabElement, panelElement, { onSurfaceReadyChange } = {}) {
    ...
    this.config = {
      // 1. 열기 타이밍 및 물리 매개변수
      openDuration: 0.3,             // 열릴 때 애니메이션 소요 시간 (초, 작을수록 빠름, 기본: 0.3)
      openBounciness: 0.9,           // 열릴 때 반동(출렁임) 세기 배율 (기본: 0.9)
    ...
    this.animationStartedAt = null; // 현재 열기/닫기 모션이 시작된 고해상도 시각
    this.physicsStepCount = 0; // 실제 프레임율과 무관하게 진행한 60Hz 고정 물리 스텝 수
  }
  ...
  /**
   * 현재 RAF 루프만 중단하고 화면에 그려진 물리 좌표는 유지합니다.
   */
  stopAnimationLoop() {
    if (this.rafId !== null) {
  ...
  /**
   * 현재 좌표에서 새 열기/닫기 모션을 시작합니다.
   * 물리는 60Hz 고정 스텝으로 계산하고, 화면에는 브라우저 RAF 주기에 맞춰 그립니다.
   */
  startAnimation() {
    if (this.rafId !== null) {
      cancelAnimationFrame(this.rafId);
  ...
  /**
   * 스프링 감쇠 알고리즘을 한 번의 60Hz 고정 시간 스텝만큼 진행합니다.
   * @param {number} duration 설정된 실제 애니메이션 소요 시간(초)
   */
  stepPhysics(duration) {
  ...
  /**
   * 실제 경과 시간을 60Hz 고정 물리 스텝 수로 환산해 프레임율과 무관하게 진행합니다.
   * 설정된 duration에 도달하면 남은 오차를 목표 좌표로 스냅하고 정확히 종료합니다.
   * @param {number} timestamp requestAnimationFrame이 전달한 고해상도 시각
   */
  tick(timestamp) {
    if (!this.animating || this.animationStartedAt === null) {
      return;
    }

    this.rafId = null;
    const duration = this.isOpen ? this.config.openDuration : this.config.closeDuration;
    const durationSeconds = Math.max(0.01, duration);
    const elapsedSeconds = Math.max(0, (timestamp - this.animationStartedAt) / 1000);
    const boundedElapsedSeconds = Math.min(elapsedSeconds, durationSeconds);
    const targetStepCount = Math.floor(
      boundedElapsedSeconds / FIXED_PHYSICS_STEP_SECONDS + 1e-9,
    );

    while (this.physicsStepCount < targetStepCount) {
      this.stepPhysics(durationSeconds);
