...
원본 `StronglyTypedExample`은 ASP.NET Core TempData 직렬화 예외를 재현하는 테스트였습니다. `readonly struct` 기반 강타입 래핑 구현은 원본에 포함되어 있지 않았습니다. 따라서 이 디렉터리는 원본을 그대로 이식한 결과가 아니라, 사용자 의도를 검증 가능한 값 객체 비교로 새로 설계한 실험입니다. 원본 TempData 예외 코드는 반입하지 않았습니다.

성능 결과는 아직 기록하지 않습니다. 강타입이 타입 혼용을 줄이는 API 경계와, 값 형식·참조 형식 표현의 비용은 같은 입력과 런타임에서 실제로 검증한 뒤에만 설명합니다.
