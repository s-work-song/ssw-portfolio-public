...
이 폴더는 포트폴리오 사이트의 연구 탭에서 다루는 실험을 재현 가능한 .NET 8 프로젝트로 정리한 공개 실험 모음입니다. 각 실험은 독립 solution을 가지므로 한 실험의 패키지나 빌드 설정이 다른 실험에 영향을 주지 않습니다.
...
| [덕 타이핑](duck-typing/README.md) | 타입 시스템 최적화 | 인터페이스, boxing, 패턴 기반 열거의 비용을 같은 의미 아래에서 비교합니다. |
| [강타입 값 객체](strongly-typed-values/README.md) | 타입 시스템 최적화 | primitive, readonly struct 값 객체, class wrapper의 타입 안전성과 비용을 비교합니다. |
...
- `mmf-vs-stream`은 현재 공개 실험에 검증 가능한 구현이 없어 포함하지 않습니다.
- `serialization-protobuf`은 ZSTD, GZip과 소켓 전송을 다루지 않으므로 압축 결과나 전송 수치를 주장하지 않습니다.
- 성능은 실행 환경에 따라 달라지므로 고정된 본실행 수치를 제공하지 않습니다. 결과는 사용자가 자신의 환경에서 실행해 확인합니다.
