...
챗봇이 받은 화면·설정 변경 지시는 브라우저에서 입력값을 검증한 뒤 실행하고,
페이지 도착이나 설정 반영을 다시 확인합니다. 브라우저 에이전트용 WebMCP는 같은
UI 실행기를 공유하지만 채팅 모델 도구와 등록 목록은 다릅니다. 요청부터 검증까지는
[챗봇·도구 실행 구조](web/docs/chatbot-architecture.md), WebMCP 11개 도구 목록과
허용값은 [웹 README](web/README.md#webmcp-도구)를 참고하세요.
